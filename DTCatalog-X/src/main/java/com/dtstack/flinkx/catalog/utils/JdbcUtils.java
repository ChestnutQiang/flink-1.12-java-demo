...
public class JdbcUtils {
    ...
    public static int executeInsertAndGetPK(
            String connUrl,
            String username,
            String pwd,
            String sql,
    ...
    public static boolean executeInsertWithTransaction(
            String connUrl, String username, String pwd, List<String> sqls) {

        List<String> columnValues = Lists.newArrayList();
        Connection conn = null;
        boolean[] isSuccess = new boolean[sqls.size()];
        try {
        ...
        try {
            ...
            for (int i = 0; i < sqls.size(); i++) {
                try (PreparedStatement ps = conn.prepareStatement(sqls.get(i))) {
                    isSuccess[i] = ps.execute();
                }
            }

            conn.commit();

            for (int i = 0; i < isSuccess.length; i++) {
                if (isSuccess[i] == false) {
                    LOG.error("The following SQL query could not be executed (%s): %s",connUrl, sqls.get(i) );
                    return false;
                }
            }
            return true;
