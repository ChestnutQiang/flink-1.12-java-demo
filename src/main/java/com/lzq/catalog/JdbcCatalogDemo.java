...
public class JdbcCatalogDemo {
    ...
    @Test
    public void runStreamSQL() throws Exception {
        ...
        String mysqlSource = "CREATE TABLE if not exists MyUserTable (\n" +
                "\tid int,\n" +
                "\tname string,\n" +
                "\tage bigint,\n" +
...
//        String printSQL = "CREATE TABLE if not exists print_table (\n" +
//                " id int,\n" +
//                " name STRING,\n" +
//                " age bigint\n" +
//                ") WITH (\n" +
...
//                ")";
        ...
        // -- 创建 Hive 表要指定 sql-dialect 为 Hive，否则创建的时候识别不了下面的 DDL 语句
        //String setSQLDialect = "SET table.sql-dialect=hive";

        String hiveYarnLogs ="CREATE TABLE if not exists dimension_table3 (\n" +
                "  id int,\n" +
                "  name STRING,\n" +
                "  age bigint \n" +
                ") PARTITIONED BY (pt_year String) TBLPROPERTIES (\n" +
        ...
        HiveCatalog hive = new HiveCatalog(name , defaultDatabase, hiveConfPath);*/

        //String insertSQL = "insert into `default_catalog`.`default_database`.print_table select id,name,age from `default_catalog`.`default_database`.MyUserTable";
//        String insertSQL = "insert into `default_catalog`.`default_database`.print_table select a.id,a.name,b.age " +
//                "from `default_catalog`.`default_database`.MyUserTable a left join dimension_table2 b on a.id = b.id";

        String insertSQL = "insert into `hiveCatalog`.`default`.print_table select b.id,b.name,b.age " +
                "from  `hiveCatalog`.`default`.dimension_table2 b ";

        String showCreateTable = "describe print_table";
        ...
        TableResult rs = tableEnv.executeSql(showCreateTable);
        rs.print();


        //tableEnv.executeSql(setSQLDialect);
        tableEnv.getConfig().setSqlDialect(SqlDialect.HIVE);
        tableEnv.executeSql(hiveYarnLogs);
